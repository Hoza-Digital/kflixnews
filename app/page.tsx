...
export default function Home() {
  return <main className="kflix-page">
    ...
    <section className="trending" aria-label="Trending stories"><strong>Trending <b>↗</b></strong><span className="trending-divider" />{trendingStories.map((s) => <a href="#latest" key={s.title}><figure><span className={`mini-still ${s.crop}`} aria-hidden="true" /><figcaption><em>{s.category}</em><span className="trending-title">{s.title}</span><small>{s.subtitle}</small></figcaption></figure></a>)}<button className="trending-next" aria-label="More trending stories">→</button></section>
