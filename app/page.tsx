...
export default function Home() {
  return <main className="kflix-page">
    ...
    <section className="hero" aria-labelledby="hero-heading"><div className="hero-copy"><h1 id="hero-heading">Culture<br /><span className="hero-motion">in motion<span className="hero-dot">.</span></span></h1><p>Entertainment news for what comes next.</p><a href="#latest" className="scroll-link">Scroll to explore <b>↓</b></a></div><div className="feature-stack" aria-label="Featured stories"><Feature crop="portal" category="FILM" title="Beyond the Horizon" copy="A new vision of tomorrow." /><Feature crop="headphones" category="MUSIC" title="Sound in Color" copy="The artist shaping the moment." /><Feature crop="gallery" category="CULTURE" title="Night to Remember" copy="Where style and story collide." /></div></section>
    <section className="trend-rail" aria-label="Trending stories"><strong className="trend-label">Trending <b>↗</b></strong><div className="trend-items">{trendingStories.map((story) => <TrendingCard key={story.title} {...story} />)}</div><button className="trend-next" aria-label="More trending stories">→</button></section>
...
function TrendingCard({ crop, category, title, subtitle }: { crop: string; category: string; title: string; subtitle: string }) { return <a className="trend-card" href="#latest"><figure><span className={`trend-thumb ${crop}`} aria-hidden="true" /><figcaption><em>{category}</em><span>{title}</span><small>{subtitle}</small></figcaption></figure></a>; }
